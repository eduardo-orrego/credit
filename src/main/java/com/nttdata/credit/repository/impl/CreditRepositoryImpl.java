...
public class CreditRepositoryImpl implements CreditRepository {
  ...
  public Mono<Credit> findCredit(BigInteger creditNumber) {
    return creditReactiveMongodb.findByCreditNumber(creditNumber)
      .doOnSuccess(credit -> log.info("Successful find - creditNumber: "
        .concat(creditNumber.toString())));
  }
  ...
  public Flux<Credit> findCredits(BigInteger customerDocument) {
    return creditReactiveMongodb.findByCustomerDocument(customerDocument)
      .doOnComplete(() -> log.info("Successful find - customerId: "
        .concat(customerDocument.toString())));
  }

  @Override
  public Mono<Boolean> findExistsCredit(String type, BigInteger customerDocument) {
    return creditReactiveMongodb.existsByTypeAndCustomerDocument(type, customerDocument)
  ...
  public Mono<Boolean> findExistsCredit(BigInteger creditNumber) {
    return creditReactiveMongodb.existsByCreditNumber(creditNumber)
      .doOnSuccess(exists -> log.info("Successful find exists - creditNumber: "
        .concat(creditNumber.toString())));
  }
  ...
  public Mono<Credit> saveCredit(Credit credit) {
    return creditReactiveMongodb.save(credit)
      .doOnSuccess(creditEntity -> log.info("Successful save - creditId: "
